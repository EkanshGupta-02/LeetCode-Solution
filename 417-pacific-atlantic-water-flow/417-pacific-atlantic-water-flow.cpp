class Solution {
    void dfs(vector<vector<int>>&heights,vector<vector<bool>>&vis,int i,int j){
        int m = heights.size();
        int n = heights[0].size();
        
        vis[i][j]=1;
        
        if(i-1 >= 0 && vis[i-1][j] != true && heights[i-1][j]>=heights[i][j]){
            dfs(heights,vis,i-1,j);
        }
        
        if(i+1 < m && vis[i+1][j] != true && heights[i+1][j]>=heights[i][j]){
            dfs(heights,vis,i+1,j);
        }
        
        if(j-1 >= 0 && vis[i][j-1] != true && heights[i][j-1]>=heights[i][j]){
            dfs(heights,vis,i,j-1);
        }
        
        if( j+1 < n && vis[i][j+1] != true && heights[i][j+1]>=heights[i][j]){
            dfs(heights,vis,i,j+1);
        }
    }
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        vector<vector<int>>ans;
        
        int  m = heights.size();
        int n = heights[0].size();
        
        if(m==0 || n==0) return {};
        
        vector<vector<bool>>atlanticvis(m,vector<bool>(n,0));
        vector<vector<bool>>pacificvis(m,vector<bool>(n,0));
        
        for(int i=0;i<m;i++){
            dfs(heights,pacificvis,i,0);
            dfs(heights,atlanticvis,i,n-1);
        }
        
        for(int j=0;j<n;j++){
            dfs(heights,pacificvis,0,j);
            dfs(heights,atlanticvis,m-1,j);
        }
        
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                if(atlanticvis[i][j] && pacificvis[i][j]){
                    ans.push_back({i,j});
                }
            }
        }
        return ans;
    }
};